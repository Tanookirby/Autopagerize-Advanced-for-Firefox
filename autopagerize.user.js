...
AutoPager.prototype.addPage = function(htmlDoc, page) {
    ...
    if (page[0] && /tr/i.test(page[0].tagName)) {
        ...
        if(settings['display_page_break']) {
...
var fixLinkRel = function() {
    var anotherElement = document.querySelector('*[class~=h-entry],*[class~=hentry]')
    ...
    var anotherElement2 = document.querySelector('*[class~=h-entry],*[class~=hentry]')
    var shouldDelete2 = anotherElement2 && anotherElement2.className.match(/(^|\s)post-.*h-?entry(\s|$)/)

    if (shouldDelete2) {
      var links2 = document.querySelectorAll('a[rel*="next"], a[rel*="Next"], a[rel*="NEXT"]')
      for(var a of links2)
          a.removeAttribute('rel')
    }
}
var fixImgThumb = function() {
    function handle(node) {
        var items = document.evaluate('.//img', node, null, 7, null);
